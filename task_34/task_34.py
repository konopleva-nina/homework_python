...
Фразы отделяются друг от друга пробелами. Стихотворение  Винни-Пух вбивает в программу с клавиатуры. В 
ответе напишите “Парам пам-пам”, если с ритмом все в порядке и “Пам парам”, если с ритмом все не в порядке'''

def ritm(stroka):
    return sum(1 for i in stroka if i in 'аеёиоуыэюя')
    
stich = input()
stroka = stich.lower().split()
temp = ritm(stroka[0])
if all([ritm(i) == temp for i in stroka]):
    print('Парам пам-пам')
else:
    print('Пам парам')